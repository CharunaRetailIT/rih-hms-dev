...
const STATUS_PILL: Record<string, string> = {
  ...
  approved: 'pill-paid',
  rejected: 'pill-void',
  // legacy value from the quick single-line adjust modal / opening-stock import
  posted: 'pill-paid',
};
...
const ACTION_CONFIG: Record<ActionKind, {
  ...
  method: 'POST' | 'DELETE';
  title: string;
  description: (a: AdjustmentRow) => string;
  confirmLabel: string;
  submittingLabel: string;
  closeLabel: string;
  danger: boolean;
  showReason: boolean;
  successMessage: (a: AdjustmentRow) => string;
}> = {
  submit: {
    endpoint: id => `/api/v1/stock-adjustments/${id}/submit`, method: 'POST',
    title: 'Submit stock adjustment',
    description: a =>
      `This submits ${a.adjustmentNumber} for processing. Depending on your approval settings it either posts straight through to stock, or waits as Pending until someone approves it.`,
    confirmLabel: 'Submit', submittingLabel: 'Submitting…', closeLabel: 'Not yet',
    danger: false, showReason: false, successMessage: a => `${a.adjustmentNumber} submitted`,
  },
  approve: {
    endpoint: id => `/api/v1/stock-adjustments/${id}/approve`, method: 'POST',
    title: 'Approve stock adjustment',
    description: a => `This approves ${a.adjustmentNumber} and posts it to stock.`,
    confirmLabel: 'Approve', submittingLabel: 'Approving…', closeLabel: 'Not now',
    danger: false, showReason: false, successMessage: a => `${a.adjustmentNumber} approved and posted`,
  },
  reject: {
    endpoint: id => `/api/v1/stock-adjustments/${id}/reject`, method: 'POST',
    title: 'Reject stock adjustment',
    description: a => `This rejects ${a.adjustmentNumber}. It never touched stock, so there's nothing to reverse.`,
    confirmLabel: 'Reject', submittingLabel: 'Rejecting…', closeLabel: "Don't reject",
    danger: true, showReason: true, successMessage: a => `${a.adjustmentNumber} rejected`,
  },
  remove: {
    endpoint: id => `/api/v1/stock-adjustments/${id}`, method: 'DELETE',
    title: 'Remove stock adjustment',
    description: a => `This removes ${a.adjustmentNumber} from the list. Only possible before it's posted to stock.`,
    confirmLabel: 'Remove', submittingLabel: 'Removing…', closeLabel: 'Keep it',
    danger: true, showReason: false, successMessage: a => `${a.adjustmentNumber} removed`,
  },
};
...
export default function StockAdjustmentsListPage() {
  ...
  const [actionFor, setActionFor] = useState<{ adj: AdjustmentRow; kind: ActionKind } | null>(null);
  ...
  async function load() {
    ...
    try {
      ...
      const res = await apiClient<PagedResponse>(`/api/v1/stock-adjustments/paged?${qs}`);
      setRows(res.data);
  ...
  return (
    <>
      <Topbar title="Stock Adjustment Notes" subtitle="Formal, approval-gated stock adjustment documents" />

      <div className="p-6">
        <div className="mb-5 flex items-center justify-between">
          <div>
            <h2 className="font-heading text-xl font-bold">Stock adjustments</h2>
            <p className="text-sm text-muted-foreground">{totalCount} adjustment{totalCount === 1 ? '' : 's'}</p>
          </div>
          <button
            onClick={() => router.push('/stock-adjustments/create')}
          ...
          >
            <Icon name="add" className="text-base" /> New Adjustment
          </button>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map((a, i) => {
                  ...
                  const submittable = SUBMITTABLE.has(a.status);
                  const pendingApproval = PENDING.has(a.status);
                  const removable = REMOVABLE.has(a.status);
                  const hasAction = editable || submittable || pendingApproval || removable;
                  return (
                    <tr key={a.id} className={i % 2 ? 'bg-muted/20' : ''}>
                      <td className="px-4 py-2.5 font-mono text-xs text-muted-foreground">{a.adjustmentNumber}</td>
                      <td className="px-4 py-2.5">
                        <span className="pill pill-idle">{a.locationCode}</span>
                      </td>
                      <td className="px-4 py-2.5 text-muted-foreground">{REASON_LABEL[a.reason] ?? a.reason}</td>
                      <td className="px-4 py-2.5">
                        <span className={`pill ${STATUS_PILL[a.status] ?? 'pill-idle'}`}>
                          {STATUS_LABEL[a.status] ?? a.status}
                        </span>
                      </td>
                      <td className="px-4 py-2.5 text-right font-semibold tabular-nums">{lkr(a.totalValue)}</td>
                      <td className="px-4 py-2.5 text-muted-foreground">
                        {a.createdAt ? new Date(a.createdAt).toLocaleDateString('en-LK') : '—'}
                      </td>
                      <td className="px-4 py-2.5 text-right">
                        <div className="flex flex-wrap items-center justify-end gap-2">
                          {editable && (
                            <button
                              onClick={() => router.push(`/stock-adjustments/${a.id}/edit`)}
                          ...
                          {submittable && (
                            <button
                              onClick={() => setActionFor({ adj: a, kind: 'submit' })}
                              className="inline-flex items-center gap-1 rounded-lg border border-border bg-card px-3 py-1.5 text-xs font-medium hover:bg-muted"
                            >
                          ...
                          {pendingApproval && (
                            <>
                              <button
                                onClick={() => setActionFor({ adj: a, kind: 'approve' })}
                              ...
                              <button
                                onClick={() => setActionFor({ adj: a, kind: 'reject' })}
                          ...
                          {removable && (
                            <button
                              onClick={() => setActionFor({ adj: a, kind: 'remove' })}
                              className="inline-flex items-center gap-1 rounded-lg border border-border bg-card px-3 py-1.5 text-xs font-medium text-status-error hover:bg-muted"
                            >
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <Pagination page={pageNumber} totalPages={totalPages} total={totalCount} from={start} to={end} setPage={setPageNumber} noun="adjustments" className="mt-0 flex-1" />
        </div>
      </div>
...
function ConfirmActionModal({
  ...
  async function submit() {
    ...
    try {
      ...
      await apiClient(cfg.endpoint(adj.id), init);
      await onDone(cfg.successMessage(adj));
    } catch (e) {
      setError((e as Error).message);
    } finally {
      setSubmitting(false);
    ...
    >
      ...
      >
        ...
        <div className="space-y-3 px-5 py-4">
          <p className="text-sm text-muted-foreground">{cfg.description(adj)}</p>
          {cfg.showReason && (
            <div>
