using RIT.HMS.Domain.ViewModels.Reports;
using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.ComponentModel.DataAnnotations;
using System.Linq;
using System.Text;
using System.Threading.Tasks;

namespace RIT.HMS.Domain.Transactions
{
  public  class AccountDataTransfer
 {
        public AccountDataTransfer()
        {
            ImportJournalDetList = new List<ImportJournalDetailsHMS>();

        }
        public List<ImportJournalDetailsHMS> ImportJournalDetList { get; set; }

        public long AccountDataTransferID { get; set; }

        [DefaultValue(0)]
        public int CompanyID { get; set; }

        [DefaultValue(0)]
        public int LocationID { get; set; }

        public string TransTypwName { get; set; }


        [DefaultValue(0)]
        public bool IsValueApply { get; set; }

        [Required()]
        public DateTime StartDate { get; set; }

        [Required()]
        public DateTime EndDate { get; set; }


        public class ImportJournalDetailsHMS
        {
            public long REFINDEX { get; set; }
            public string EXBATCH { get; set; }
            public string TRANTYPE { get; set; }
            public string DOCNO { get; set; }
            public string DOCNO1 { get; set; }
            public DateTime? DATE { get; set; }
            public DateTime? DUEDATE { get; set; }
            public long SEQNO { get; set; }
            public string ACODE { get; set; }
            public string CCODE { get; set; }
            public string DRCR { get; set; }
            public string DESCRIPTION { get; set; }
            public decimal AMOUNT { get; set; }
            public string CQNO { get; set; }
            public DateTime? CQDATE { get; set; }
            public string BANK { get; set; }
            public string BANKBRANCH { get; set; }
            public bool PROCESS { get; set; }
            public bool GLPOST { get; set; }
            public string GLPOSTUSER { get; set; }
            public DateTime? GLPOSTDATETIME { get; set; }
            public string GLPOSTCPNAME { get; set; }
            public int CUSTOMER { get; set; }
            public int CUSTOMERCODE { get; set; }
            public int SUPPLIER { get; set; }
            public bool ISTAX { get; set; }
            public int ADDITION { get; set; }
            public int DEDUCTION { get; set; }
            public bool ISPAIDIN { get; set; }
            public bool ISPAIDOUT { get; set; }
            public bool ISCREDITED { get; set; }

        }




    }
}
